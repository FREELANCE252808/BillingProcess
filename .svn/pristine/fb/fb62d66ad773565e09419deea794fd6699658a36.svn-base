...
import { UsersRegistrationModel } from '../models/userRegistration.model';
...
export class AuthenticationService {
	...
	public login(credential: Credential): Observable<any> {
		...
		return this.http.get<AccessData>("api/Login/auth/LoginAsync" + '?' + this.util.urlParam(credential), { headers: headers, params: params }).pipe(
			map((result: any) => {
	...
	public IsEmailIDRegister(credential: Credential): Observable<any> {
		debugger
		let params = new HttpParams();
		params.set('email', credential.email);
		params.set('password', credential.password);
		const headers = new HttpHeaders()
			.set("Content-Type", "application/json; charset=utf-8");
		return this.http.get<AccessData>("api/Login/auth/IsEmailIDRegister" + '?' + this.util.urlParam(credential), { headers: headers, params: params }).pipe(
			map((result: any) => {
				if (result instanceof Array) {
					return result.pop();
				}
	...
	public GetUserNameByEmailID(credential: Credential): Observable<any> {
		debugger
		let params = new HttpParams();
		params.set('email', credential.email);
		params.set('password', credential.password);
		const headers = new HttpHeaders()
			.set("Content-Type", "application/json; charset=utf-8");
		return this.http.get<AccessData>("api/Login/auth/GetUserNameByEmailID" + '?' + this.util.urlParam(credential), { headers: headers, params: params }).pipe(
			map((result: any) => {
				if (result instanceof Array) {
					return result.pop();
				}
	...
	//GetUserNameByEmailID(email: Credential): Observable<any[]> {
	//	debugger;
	//	return this.http.get<any[]>("api/Login/auth/GetUserNameByEmailID?email=" + email);
	//}
	 ...
	 * @returns {Observable<any>}
	 */
	public register(model: UsersRegistrationModel): Observable<any> {
		...
		//this.http.post("http://localhost:5050api/auth/login", credentials, {
		...
		const headers = new HttpHeaders()
			.set("Content-Type", "application/json; charset=utf-8");
		return this.http.post<AccessData>("api/Login/auth/Register", model, this.util.getHTTPHeader()).pipe(
			map((result: any) => {
				if (result instanceof Array) {
					return result.pop();
				}
	...
	public requestPassword(credential: Credential): Observable<any> {
		return this.http.get('api/User/auth/RequestPassword?' + this.util.urlParam(credential))
